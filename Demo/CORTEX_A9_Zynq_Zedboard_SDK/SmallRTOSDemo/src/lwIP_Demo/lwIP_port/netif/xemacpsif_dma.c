...
#ifdef OS_IS_SMALLRTOS
extern sOSBase_t xInsideISR;
#endif

#define XEMACPS_BD_TO_INDEX(ringptr, bdptr)				\
	(((u32)bdptr - (u32)(ringptr)->BaseBdAddr) / (ringptr)->Separation)

sys_mbox_t TxRingMsgQ = NULL;//Add for app
...
void vPendableSendCompleteTask(void* pvParameter)
{

	XEmacPs_BdRing* TxRingPtr;
	for(;;)
	{
		sys_arch_mbox_fetch(&TxRingMsgQ, (void**)&TxRingPtr, 0);
		if(TxRingPtr != NULL)
		{
...
void emacps_send_handler(void *arg)
{
	...
	obtaining the mutex fails. */
	{
		...
		{
			sys_mbox_trypost(&TxRingMsgQ, TxRingPtr);
		}
	}
